...
const filteredCount = computed(() => posts.value.length);
...
<template>
	<div>
			...
			<template v-if="posts?.length">
				...
				>
					...
					<div class="p-4">
						...
						<p v-if="getTranslation(post, 'description')" class="text-sm text-foreground mt-2">
		...
		<ClientOnly>
			<Pagination v-if="totalPages > 1 && posts?.length && data.show_pagination" class="mt-6">
