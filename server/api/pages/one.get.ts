	...
	try {
							...
							blocks: [
								...
								'hide_block',
								{
									item: {
										...
										block_audio: ['id', 'tagline', 'headline', { items: ['id', 'directus_files_id', 'sort'] }],
										block_pricing: [
											'id',
											'tagline',
											'headline',
		...
		if (Array.isArray(page?.blocks)) {
			for (const block of page.blocks as PageBlock[]) {
				...
				if (
					block.collection === 'block_members' &&
					block.item &&
					typeof block.item !== 'string' &&
					'collection' in block.item &&
					block.item.collection === 'members'
				) {
				...
				if (
					block.collection === 'block_disciplines' &&
					block.item &&
					typeof block.item !== 'string' &&
					'collection' in block.item &&
					block.item.collection === 'disciplines'
				) {
